        ...
        <aside class="w-64 glass-panel border-r border-gray-800 flex flex-col h-full bg-slate-900/50 relative z-20">
            <div class="px-6 py-8 border-b border-gray-800 flex flex-col gap-4">
                ...
                @if(session('room_id') || session('empresa_room_id') || auth()->user()->isSocio() || auth()->user()->isEmpresa())
                    ...
                    <a href="{{ route('facturacion.index') }}" class="flex items-center px-4 py-3 rounded-2xl transition-all duration-300 {{ request()->routeIs('facturacion.*') ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:bg-slate-800 hover:text-white' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        <span class="font-bold text-sm">{{ __('Facturación y Cierres') }}</span>
                    </a>
