...
<div class="glass-panel p-6 rounded-xl max-w-2xl mx-auto mt-8">
    <h3 class="text-xl font-semibold mb-6">Editando Pasajero: {{ $pasajero->nombre_completo }}</h3>
    ...
    <form action="{{ route('pasajeros.update', $pasajero->id) }}" method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="nombre_completo" value="{{ $pasajero->nombre_completo }}" class="mt-1 block w-full rounded-md bg-slate-900 border-slate-700 text-white shadow-sm px-4 py-2">
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-300">Número de Teléfono</label>
                <input type="text" name="telefono" value="{{ $pasajero->telefono }}" placeholder="+54911..." class="mt-1 block w-full rounded-md bg-slate-900 border-slate-700 text-white shadow-sm px-4 py-2 font-mono text-emerald-300">
            ...
            <div>
                ...
                <input type="text" name="documento" value="{{ $pasajero->documento }}" class="mt-1 block w-full rounded-md bg-slate-900 border-slate-700 text-white shadow-sm px-4 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-300">¿A qué Centro de Costo pertenece?</label>
                <select name="centro_costo_id" class="mt-1 block w-full rounded-md bg-slate-900 border-slate-700 text-white shadow-sm px-4 py-2">
                    @foreach($centros as $centro)
                        <option value="{{ $centro->id }}" {{ $pasajero->centro_costo_id == $centro->id ? 'selected' : '' }}>
        ...
        <div class="mt-8 flex gap-4">
            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-md font-semibold transition">Guardar Cambios</button>
